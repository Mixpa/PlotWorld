...
public class Road{
    public static ChunkGenerator.ChunkData getChunkDate(ChunkGenerator.ChunkData chunkData, ChunkGenerator.BiomeGrid biome) {
        ...
        //石头
        chunkData.setRegion(0, 1, 0, 16, PlotConfig.getWorldHeight(), 16, Material.STONE);
        //砖头
        chunkData.setRegion(0, PlotConfig.getWorldHeight(), 0, 16, PlotConfig.getWorldHeight() + 1, 16, Material.SMOOTH_BRICK);
        //设置整个区块为虚空
        for (int i = 0; i < 16; i++) {
            for (int j = 0; j < 16; j++) {
